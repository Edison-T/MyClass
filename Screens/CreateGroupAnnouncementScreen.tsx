...
import { getGroupRoom } from "./queries";


const CreateGroupAnnouncementScreen = ({route}, props) => {
    ...
    useLayoutEffect (() => {
        ...
        useEffect(() => {
            ...
            try {
                ...
                setPushTokens(userData.data.getGroupRoom.groupRoomUsers.items.map((u) => u.user.expoPushToken))
            } catch (e) {
                console.log(e);
            }} 
            getCRUTokens();
        ...
        const sendPushNotification = () => {
            let response = fetch('https://exp.host/--/api/v2/push/send', {
              ...
              body: JSON.stringify({
                to: pushTokens,
                sound: 'default',
                title: `Announcement | ${groupRoomName}`,
                body: `"${message}"`
              })
    ...
    const sendAnnouncment = async () => {
        const newAnnouncementData = await API.graphql(
            graphqlOperation(
                mutations.createGroupAnnouncement, {
                    input: {
        ...
        await updateGroupRoomLastAnnouncement(newAnnouncementData.data.createGroupAnnouncement.id);
        await updateGRULastAnnouncement(newAnnouncementData.data.createGroupAnnouncement.updatedAt);
        sendPushNotification();
        navigation.navigate('Group Announcements')
    }  
    
    const updateGroupRoomLastAnnouncement = async (announcementId: string) => {
        try {
            await API.graphql(
                graphqlOperation(
                    mutations.updateGroupRoom, {
                        input: {
                            id: groupRoomID,
    ...
    const updateGRULastAnnouncement = async (timeStamp: String) => {
        try {
            await API.graphql(
                graphqlOperation(
...
export default CreateGroupAnnouncementScreen;