...
import { Text, View, Image, TouchableOpacity, KeyboardAvoidingView, Platform, Pressable, FlatList, ScrollView } from "react-native";
...
import { createConversationMessage, createGroupRoomConversationMessage, createMessage, updateChatRoom, updateChatRoomUser, updateConversation, updateConversationUser, updateGroupRoomConversation, updateGroupRoomConversationUser } from "../../src/graphql/mutations";
...
const GroupRoomConversationInputBox = (props, ) => {
    ...
    const {groupRoomConversationID, messageReplyTo, removeMessageReplyTo, CRUID, usersTokens, groupRoomConversationName, myId, myName, inputBoxListUsers } = props;
    ...
    const updateConversationLastMessage = async (messageId: string) => {
        try {
            await API.graphql(
                graphqlOperation(
        ...
        if (messageReplyTo) {
            let response = fetch('https://exp.host/--/api/v2/push/send', {
                ...
                body: JSON.stringify({
                  ...
                  sound: 'default',
                  title: `${groupRoomConversationName}`,
    ...
    const onSendPress = async () => {
        ...
        try {
            ...
            await updateConversationLastMessage(newMessageData.data.createGroupRoomConversationMessage.id);
            await updateCRLastMessage(newMessageData.data.createGroupRoomConversationMessage.updatedAt);
            if (arrayOfPushTokens) {
...
export default GroupRoomConversationInputBox